...
const tempData = ref([]);
const renderedMarkdownToc: Ref<any> = ref([]);
...
const renderMarkdown = () => {
      ...
      render: (tokens: any, idx: any) => {
        ...
        if (token.nesting === 1) {
          ...
          return container !== "details"
            ? `<div class="${container}-container"><p class="cu-title">${
                container === "tip"
                  ? "提示"
                  : container === "warning"
                  ? "注意"
  ...
  // 自定义摘要的渲染逻辑
  md.renderer.rules.details_summary = function (tokens: any, idx: any) {
    const token = tokens[idx];
    return (
  ...
  md.renderer.rules.heading_open = (
    ...
    tempData.value.push({ level, anchor: "toc_" + anchor, titleText } as never);
    renderedMarkdownToc.value = buildTree(tempData.value);
    return `<h${level} id="toc_${anchor}">`;
  };

  // 构建树状结构的函数
  function buildTree(dataList: any) {
